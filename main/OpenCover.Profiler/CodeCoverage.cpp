...
HRESULT STDMETHODCALLTYPE CCodeCoverage::ObjectAllocated( 
	/* [in] */ ObjectID objectId,
	/* [in] */ ClassID classId)
{ 
	ATLTRACE(_T(" ::ObjectAllocated() objectId: 0x%X classId: 0x%X"), objectId, classId);
	return S_OK; 
}
...
HRESULT STDMETHODCALLTYPE CCodeCoverage::ClassLoadFinished( 
	...
	/* [in] */ HRESULT hrStatus) 
{ 
	ATLTRACE(_T(" ::ClassLoadFinished() classId: 0x%X hrStatus: 0x%X"), classId, hrStatus);

	ModuleID moduleId;
	mdTypeDef typeDefToken;
	HRESULT hr = m_profilerInfo->GetClassIDInfo(classId, &moduleId, &typeDefToken);
	if (!SUCCEEDED(hr))
	{
		RELTRACE(_T("    GetClassIDInfo failed 0x%X"), hr);
	}

	ATLTRACE(_T(" ClassIDInfo classId: 0x%X hrStatus: 0x%X"), classId, hrStatus);

	if (SUCCEEDED(hr) && typeDefToken!=0)
	{
		CComPtr<IMetaDataImport> metaDataImport;
		COM_FAIL_MSG_RETURN_ERROR(m_profilerInfo->GetModuleMetaData(moduleId, 
		...
		ULONG dwNameSize = 1024;
		WCHAR szTypeName[1024] = {};
		DWORD dwtypeDefFlags;
		mdToken extendsToken;

		hr = metaDataImport->GetTypeDefProps(typeDefToken, szTypeName, dwNameSize, &dwNameSize, &dwtypeDefFlags, &extendsToken);

		ATLTRACE(_T(" Class name '%s'"), szTypeName);
	}
